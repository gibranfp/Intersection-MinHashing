...
{
     HashTable htable1;
     imh_init_table(&htable1);
     imh_print_head(&htable1);
     imh_print_table(&htable1);

	
     HashTable htable2 = imh_create_table(256, 3, 100, 4);
     imh_print_head(&htable2);

     imh_generate_permutations(htable2.dim, htable2.tuple_size, htable2.permutations);
...
void test_hash(void)
{
     HashTable htable = imh_create_table(256, 3, 12, 4);
     imh_print_head(&htable);

     imh_generate_permutations(htable.dim, htable.tuple_size, htable.permutations);
     ...
     for (i = 0; i < htable.tuple_size; i++) {
          ...
          uint index = imh_compute_minhash(&list, &htable.permutations[i * htable.dim]);
          printf("\n\tindex[%u] = %u\n", i, index);
     }

     uint hash_value, bucket1, bucket2;
     imh_compute_univhash(&list, &htable, &hash_value, &bucket1);
     bucket2 = imh_get_index(&list, &htable);
...
void test_store(void)
{
     ...
     HashTable htable = imh_create_table(256, 3, 12, 2);
     imh_generate_permutations(htable.dim, htable.tuple_size, htable.permutations);
     uint index = imh_store_list(&list, 0, &htable);
     uint i;
     printf("Index = %u\n", index);
     ...
     imh_store_listdb(&listdb, &htable, indices);
...
void test_iminhash_binary(uint number_of_hashes)
{
     ...
     imh_generate_permutations(listdb.dim, number_of_hashes, permutations);

     HashTable hash_table = imh_create_table(256, 3, 12, 2);;
     uint *sublist_number;
     uint sublistdb_size = imh_get_sublist_numbers(&listdb,
                                                   hash_table.sublist_size,
                                                   sublist_number);
     
     uint *sublist_ids = malloc(sublistdb_size * sizeof(uint));
     ListDB sublistdb = imh_create_sublistdb_from_listdb(&listdb,
                                                         sublist_number,
                                                         sublistdb_size,
                                                         sublist_ids);
     
     ullong *mhdb = calloc(number_of_hashes * sublistdb.size, sizeof(ullong));
     for (i = 0; i < sublistdb.size; i++) {
          for (j = 0; j < number_of_hashes; j++)
               mhdb[i * number_of_hashes + j] = imh_compute_minhash(&sublistdb.lists[i],
                                                                    &permutations[j * sublistdb.dim]);
     }
     ...
     for (i = 0; i < listdb.size - 1; i++)
          for (j = i + 1; j < listdb.size; j++) {
               dist[i][j] = list_overlap(&listdb.lists[i], &listdb.lists[j]);
...
/*      uint *maxfreq = imh_get_cumulative_frequency(&listdb, &ifindex); */
/*      ListDB newdb = imh_expand_listdb(&listdb, maxfreq); */
/*      free(maxfreq); */

/*      RandomValue *permutations = (RandomValue *) malloc(number_of_hashes * newdb.dim * sizeof(RandomValue)); */
/*      imh_generate_permutations(newdb.dim, number_of_hashes, permutations); */

/*      ullong *mhdb = (ullong *) calloc(number_of_hashes * listdb.size, sizeof(ullong)); */
/*      for (i = 0; i < listdb.size; i++) { */
/*           for (j = 0; j < number_of_hashes; j++)  */
/*                mhdb[i * number_of_hashes + j] = imh_compute_minhash(&newdb.lists[i], &permutations[j * newdb.dim]); */
/*      } */
...
/*      RandomValue *permutations = (RandomValue *) malloc(number_of_hashes * listdb.dim * sizeof(RandomValue)); */
/*      imh_generate_permutations(listdb.dim, number_of_hashes, permutations); */
/*      imh_weight_permutations(listdb.dim, number_of_hashes, permutations, weights); */
     
/*      ullong *mhdb = (ullong *) calloc(number_of_hashes * listdb.size, sizeof(ullong)); */
/*      for (i = 0; i < listdb.size; i++) { */
/*           for (j = 0; j < number_of_hashes; j++)  */
/*                mhdb[i * number_of_hashes + j] = imh_compute_minhash(&listdb.lists[i], &permutations[j * listdb.dim]); */
/*      } */
...
/*      double *weights = imh_expand_weights(ifindex.size, maxfreq, oldweights); */
/*      free(maxfreq); */
     
/*      RandomValue *permutations = (RandomValue *) malloc(number_of_hashes * newdb.dim * sizeof(RandomValue)); */
/*      imh_generate_permutations(newdb.dim, number_of_hashes, permutations); */
/*      imh_weight_permutations(newdb.dim, number_of_hashes, permutations, weights); */
...
/*      ListDB hashes = listdb_createlistdb.size, 0); */
/*      for (i = 0; i < listdb.size; i++) { */
/*           for (j = 0; j < number_of_hashes; j++)  */
/*                mhdb[i * number_of_hashes + j] = imh_compute_minhash(&listdb.lists[i], &permutations[j * listdb.dim]); */
/*      } */
